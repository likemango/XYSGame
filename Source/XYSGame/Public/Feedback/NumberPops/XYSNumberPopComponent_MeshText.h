...
#include "XYSNumberPopComponent_MeshText.generated.h"

class UXYSDamagePopStyle;
...
class UXYSNumberPopComponent_MeshText : public UXYSNumberPopComponent
{
	GENERATED_BODY()

public:

	UXYSNumberPopComponent_MeshText(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	void SetMaterialParameters(const FXYSNumberPopRequest& Request, FTempNumberPopInfo& NewDamageNumberInfo, const FTransform& CameraTransform, const FVector& NumberLocation);

	FLinearColor DetermineColor(const FXYSNumberPopRequest& Request) const;
	UStaticMesh* DetermineStaticMesh(const FXYSNumberPopRequest& Request) const;
	...
	TArray<TObjectPtr<UXYSDamagePopStyle>> Styles;
