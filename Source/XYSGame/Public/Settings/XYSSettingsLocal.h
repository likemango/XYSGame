...
#include "XYSSettingsLocal.generated.h"

class USoundControlBusMix;
class USoundControlBus;

USTRUCT()
struct FXYSScalabilitySnapshot
{
	GENERATED_BODY()

	FXYSScalabilitySnapshot();
 ...
 */
UCLASS()
class XYSGAME_API UXYSSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EXYSStatDisplayMode GetPerfStatDisplayState(EXYSDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EXYSDisplayablePerformanceStat Stat, EXYSStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UXYSSettingsLocal, FPerfStatSettingsChanged);
	...
	DECLARE_EVENT(UXYSSettingsLocal, FLatencyFlashInidicatorSettingChanged);
	UFUNCTION()
	...
	DECLARE_EVENT(UXYSSettingsLocal, FLatencyStatEnabledSettingChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EXYSDisplayablePerformanceStat, EXYSStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FXYSScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FXYSScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UXYSSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
