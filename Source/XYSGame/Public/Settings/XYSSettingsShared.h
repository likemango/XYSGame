...
UENUM(BlueprintType)
enum class EXYSAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM(BlueprintType)
enum class EXYSGamepadSensitivity : uint8
{
 ...
 */
UCLASS()
class XYSGAME_API UXYSSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

	public:
	DECLARE_EVENT_OneParam(UXYSSettingsShared, FOnSettingChangedEvent, UXYSSettingsShared* Settings);
...
public:
	...
	static UXYSSettingsShared* CreateTemporarySettings(const UXYSLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UXYSSettingsShared* LoadOrCreateSettings(const UXYSLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UXYSSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UXYSLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EXYSAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EXYSAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EXYSAllowBackgroundAudioSetting AllowAudioInBackground = EXYSAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EXYSGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EXYSGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EXYSGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EXYSGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EXYSGamepadSensitivity GamepadLookSensitivityPreset = EXYSGamepadSensitivity::Normal;
	UPROPERTY()
	EXYSGamepadSensitivity GamepadTargetingSensitivityPreset = EXYSGamepadSensitivity::Normal;
