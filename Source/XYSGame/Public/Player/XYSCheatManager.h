...
#include "XYSCheatManager.generated.h"
...
DECLARE_LOG_CATEGORY_EXTERN(LogXYSCheat, Log, All);

/**
 * 
 */
UCLASS(config = Game, Within = PlayerController)
class XYSGAME_API UXYSCheatManager : public UCheatManager
{
	GENERATED_BODY()
...
protected:
	...
	void ApplySetByCallerDamage(UXYSAbilitySystemComponent* XYSASC, float DamageAmount);
	void ApplySetByCallerHeal(UXYSAbilitySystemComponent* XYSASC, float HealAmount);

	UXYSAbilitySystemComponent* GetPlayerAbilitySystemComponent() const;
