...
#include "XYSWeaponStateComponent.generated.h"
...
struct FXYSScreenSpaceHitLocation
{
...
struct FXYSServerSideHitMarkerBatch
{
	FXYSServerSideHitMarkerBatch() { }

	FXYSServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FXYSScreenSpaceHitLocation> Markers;
...
class XYSGAME_API UXYSWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UXYSWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FXYSScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FXYSScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FXYSServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
