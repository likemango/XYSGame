...
#include "XYSAudioMixEffectsSubsystem.generated.h"

class USoundControlBusMix;
class USoundControlBus;

USTRUCT()
struct FXYSAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 */
UCLASS()
class XYSGAME_API UXYSAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	// Default Sound Control Bus Mix retrieved from the XYS Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the XYS Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the XYS Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> UserMix = nullptr;

	// Overall Sound Control Bus retrieved from the XYS Audio Settings and linked to the UI and game settings in XYSSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> OverallControlBus = nullptr;

	// Music Sound Control Bus retrieved from the XYS Audio Settings and linked to the UI and game settings in XYSSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> MusicControlBus = nullptr;

	// SoundFX Sound Control Bus retrieved from the XYS Audio Settings and linked to the UI and game settings in XYSSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> SoundFXControlBus = nullptr;

	// Dialogue Sound Control Bus retrieved from the XYS Audio Settings and linked to the UI and game settings in XYSSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> DialogueControlBus = nullptr;

	// VoiceChat Sound Control Bus retrieved from the XYS Audio Settings and linked to the UI and game settings in XYSSettingsLocal
	UPROPERTY(Transient)
	...
	UPROPERTY(Transient)
	TArray<FXYSAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FXYSAudioSubmixEffectsChain> LDRSubmixEffectChain;
