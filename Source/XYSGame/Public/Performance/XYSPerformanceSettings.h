...
#include "XYSPerformanceSettings.generated.h"
...
USTRUCT()
struct FXYSQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FXYSPerformanceStatGroup
{
	GENERATED_BODY()
...
class XYSGAME_API UXYSPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UXYSPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UXYSPlatformSpecificRenderingSettings* Get();
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FXYSQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EXYSFramePacingMode FramePacingMode = EXYSFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by XYS.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EXYSFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="XYS Performance Settings"))
class UXYSPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UXYSPerformanceSettings();
...
public:
	...
	TArray<FXYSPerformanceStatGroup> UserFacingPerformanceStats;
};