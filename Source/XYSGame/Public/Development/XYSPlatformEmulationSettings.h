...
#include "XYSPlatformEmulationSettings.generated.h"

/**
 * 
 */
UCLASS(config=EditorPerProjectUserSettings)
class XYSGAME_API UXYSPlatformEmulationSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

	
public:
	UXYSPlatformEmulationSettings();
...
private:
	...
	// The base device profile to pretend we are using when emulating device-specific device profiles applied from UXYSSettingsLocal
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="XYS.Settings.ApplyFrameRateSettingsInPIE"))
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="XYS.Settings.ApplyFrontEndPerformanceOptionsInPIE"))
	bool bApplyFrontEndPerformanceOptionsInPIE = false;

	// Should we apply experience/platform emulated device profiles in PIE?
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(InlineEditConditionToggle, ConsoleVariable="XYS.Settings.ApplyDeviceProfilesInPIE"))
