...
#include "Weapon/XYSWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(XYSRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_XYS_Weapon_SteadyAimingCamera, "XYS.Weapon.SteadyAimingCamera");

UXYSRangedWeaponInstance::UXYSRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UXYSRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UXYSRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UXYSRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UXYSRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UXYSRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UXYSRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UXYSRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UXYSRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UXYSRangedWeaponInstance::AddSpread()
{
...
float UXYSRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UXYSRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UXYSRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	//
	// 	AimingAlpha = (TopCameraTag == TAG_XYS_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	// }
