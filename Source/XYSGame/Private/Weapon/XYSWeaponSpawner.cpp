...
#include "Weapon/XYSWeaponSpawner.h"

#include "AbilitySystemBlueprintLibrary.h"
#include "Components/CapsuleComponent.h"
#include "Components/StaticMeshComponent.h"
#include "Engine/World.h"
#include "Equipment/Pickup/XYSPickupDefinition.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(XYSWeaponSpawner)
...
// Sets default values
AXYSWeaponSpawner::AXYSWeaponSpawner()
{
	...
	CollisionVolume->OnComponentBeginOverlap.AddDynamic(this, &AXYSWeaponSpawner::OnOverlapBegin);
...
void AXYSWeaponSpawner::BeginPlay()
{
	Super::BeginPlay();
	...
	else if (const UWorld* World = GetWorld())
	{
		...
		{
			UE_LOG(LogXYSGame, Error, TEXT("'%s' does not have a valid weapon definition! Make sure to set this data on the instance!"), *GetNameSafe(this));	
		}
	}
}

void AXYSWeaponSpawner::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void AXYSWeaponSpawner::Tick(float DeltaTime)
{
...
void AXYSWeaponSpawner::OnConstruction(const FTransform& Transform)
{
...
void AXYSWeaponSpawner::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
...
void AXYSWeaponSpawner::CheckForExistingOverlaps()
{
...
void AXYSWeaponSpawner::AttemptPickUpWeapon_Implementation(APawn* Pawn)
{
	if (GetLocalRole() == ROLE_Authority && bIsWeaponAvailable && UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(Pawn))
	{
		TSubclassOf<UXYSInventoryItemDefinition> WeaponItemDefinition = WeaponDefinition ? WeaponDefinition->InventoryItemDefinition : nullptr;
...
void AXYSWeaponSpawner::StartCoolDown()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().SetTimer(CoolDownTimerHandle, this, &AXYSWeaponSpawner::OnCoolDownTimerComplete, CoolDownTime);
	}
}

void AXYSWeaponSpawner::ResetCoolDown()
{
	UWorld* World = GetWorld();
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		if (World)
		{
			World->GetTimerManager().SetTimer(CheckOverlapsDelayTimerHandle, this, &AXYSWeaponSpawner::CheckForExistingOverlaps, CheckExistingOverlapDelay);
		}
	}
...
void AXYSWeaponSpawner::OnCoolDownTimerComplete()
{
	ResetCoolDown();
}

void AXYSWeaponSpawner::SetWeaponPickupVisibility(bool bShouldBeVisible)
{
	WeaponMesh->SetVisibility(bShouldBeVisible, true);
}

void AXYSWeaponSpawner::PlayPickupEffects_Implementation()
{
...
void AXYSWeaponSpawner::PlayRespawnEffects_Implementation()
{
	if (WeaponDefinition != nullptr)
	{
...
void AXYSWeaponSpawner::OnRep_WeaponAvailability()
{
...
void AXYSWeaponSpawner::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(AXYSWeaponSpawner, bIsWeaponAvailable);
}

int32 AXYSWeaponSpawner::GetDefaultStatFromItemDef(const TSubclassOf<UXYSInventoryItemDefinition> WeaponItemClass, FGameplayTag StatTag)
{
	if (WeaponItemClass != nullptr)
	{
		if (UXYSInventoryItemDefinition* WeaponItemCDO = WeaponItemClass->GetDefaultObject<UXYSInventoryItemDefinition>())
		{
