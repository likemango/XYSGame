...
UXYSWeaponInstance::UXYSWeaponInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	if (APawn* Pawn = GetPawn())
	{
		...
		{
			if (UXYSHealthComponent* HealthComponent = UXYSHealthComponent::FindHealthComponent(GetPawn()))
			{
...
void UXYSWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UXYSWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
...
void UXYSWeaponInstance::UpdateFiringTime()
{
	UWorld* World = GetWorld();
	check(World);
...
float UXYSWeaponInstance::GetTimeSinceLastInteractedWith() const
{
	UWorld* World = GetWorld();
	check(World);
...
void UXYSWeaponInstance::ApplyDeviceProperties()
{
...
void UXYSWeaponInstance::RemoveDeviceProperties()
{
	const FPlatformUserId UserId = GetOwningUserId();
...
void UXYSWeaponInstance::OnDeathStarted(AActor* OwningActor)
{
