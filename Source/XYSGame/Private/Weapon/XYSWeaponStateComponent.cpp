...
// #include "Teams/XYSTeamSubsystem.h"
#include "Weapon/XYSRangedWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(XYSWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

UXYSWeaponStateComponent::UXYSWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void UXYSWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);

	if (APawn* Pawn = GetPawn<APawn>())
	{
		if (UXYSEquipmentComponent* EquipmentManager = Pawn->FindComponentByClass<UXYSEquipmentComponent>())
		{
			if (UXYSRangedWeaponInstance* CurrentWeapon = Cast<UXYSRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(UXYSRangedWeaponInstance::StaticClass())))
			{
...
bool UXYSWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
	AActor* HitActor = Hit.GetActor();

	//@TODO: Don't treat a hit that dealt no damage (due to invulnerability or similar) as a success
	UWorld* World = GetWorld();
	// if (UXYSTeamSubsystem* TeamSubsystem = UWorld::GetSubsystem<UXYSTeamSubsystem>(GetWorld()))
	// {
...
bool UXYSWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void UXYSWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		FXYSServerSideHitMarkerBatch& Batch = UnconfirmedServerSideHitMarkers[i];
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FXYSScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void UXYSWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FXYSServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FXYSScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void UXYSWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void UXYSWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double UXYSWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
