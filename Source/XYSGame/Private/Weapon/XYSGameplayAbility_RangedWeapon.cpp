...
#include "Weapon/XYSGameplayAbility_RangedWeapon.h"
...
#include "Weapon/XYSWeaponStateComponent.h"
...
namespace XYSConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("XYS.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("XYS.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("XYS.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UXYSGameplayAbility_RangedWeapon::UXYSGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
...
bool UXYSGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogXYSAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UXYSRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UXYSGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UXYSGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UXYSGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return TraceChannel_Weapon;
}

FHitResult UXYSGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FTransform UXYSGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EXYSAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UXYSWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UXYSWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EXYSAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EXYSAbilityTargetingSource::CameraTowardsFocus) || (Source == EXYSAbilityTargetingSource::PawnTowardsFocus) || (Source == EXYSAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EXYSAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EXYSAbilityTargetingSource::WeaponForward) || (Source == EXYSAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EXYSAbilityTargetingSource::PawnTowardsFocus) || (Source == EXYSAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UXYSGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (XYSConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, XYSConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UXYSGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UXYSRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		if (XYSConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, XYSConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UXYSGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UXYSRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (XYSConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, XYSConsoleVariables::DrawBulletHitRadius, FColor::Red, false, XYSConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UXYSGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UXYSGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UXYSGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UXYSWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UXYSWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogXYSAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UXYSGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UXYSWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UXYSWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FXYSGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FXYSGameplayAbilityTargetData_SingleTargetHit();
