...
{
	for (const FXYSHUDElementEntry& Entry : Widgets)
	{
...
EDataValidationResult UGameFeatureAction_AddWidgets::IsDataValid(FDataValidationContext& Context) const
{
	...
	{
		int32 EntryIndex = 0;
		for (const FXYSHUDLayoutRequest& Entry : Layout)
		{
...
void UGameFeatureAction_AddWidgets::AddToWorld(const FWorldContext& WorldContext, const FGameFeatureStateChangeContext& ChangeContext)
{
	...
	if ((GameInstance != nullptr) && (World != nullptr) && World->IsGameWorld())
	{
		...
		{			
			TSoftClassPtr<AActor> HUDActorClass = AXYSHUD::StaticClass();

			TSharedPtr<FComponentRequestHandle> ExtensionRequestHandle = ComponentManager->AddExtensionHandler(
				HUDActorClass, UGameFrameworkComponentManager::FExtensionHandlerDelegate::CreateUObject(this, &ThisClass::HandleActorExtension, ChangeContext));
...
{
	AXYSHUD* HUD = CastChecked<AXYSHUD>(Actor);
