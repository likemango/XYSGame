...
#include UE_INLINE_GENERATED_CPP_BY_NAME(XYSBlueprintFunctionLibrary)

TSoftObjectPtr<UObject> UXYSBlueprintFunctionLibrary::GetTypedSoftObjectReferenceFromPrimaryAssetId(FPrimaryAssetId PrimaryAssetId, TSubclassOf<UObject> ExpectedAssetType)
{
	if (UAssetManager* Manager = UAssetManager::GetIfInitialized())
	{
		...
		if (Manager->GetPrimaryAssetTypeInfo(PrimaryAssetId.PrimaryAssetType, Info) && !Info.bHasBlueprintClasses)
		{
			...
			{
				UE_LOG(LogXYSGame, Warning, TEXT("GetTypedSoftObjectReferenceFromPrimaryAssetId(%s, %s) - AssetBaseClassLoaded was unset so we couldn't validate it, returning null"),
...
// FPrimaryAssetId UXYSBlueprintFunctionLibrary::GetPrimaryAssetIdFromUserFacingExperienceName(const FString& AdvertisedExperienceID)
// {
// 	const FPrimaryAssetType Type(UXYSUserFacingExperienceDefinition::StaticClass()->GetFName());
// 	return FPrimaryAssetId(Type, FName(*AdvertisedExperienceID));
// }

void UXYSBlueprintFunctionLibrary::PlayNextGame(const UObject* WorldContextObject)
{
...
void UXYSBlueprintFunctionLibrary::SetScalarParameterValueOnAllMeshComponents(AActor* TargetActor, const FName ParameterName, const float ParameterValue, bool bIncludeChildActors)
{
...
void UXYSBlueprintFunctionLibrary::SetVectorParameterValueOnAllMeshComponents(AActor* TargetActor, const FName ParameterName, const FVector ParameterValue, bool bIncludeChildActors)
{
	if (TargetActor != nullptr)
	{
		TargetActor->ForEachComponent<UMeshComponent>(bIncludeChildActors, [=](UMeshComponent* InComponent)
...
void UXYSBlueprintFunctionLibrary::SetColorParameterValueOnAllMeshComponents(AActor* TargetActor, const FName ParameterName, const FLinearColor ParameterValue, bool bIncludeChildActors)
{
	SetVectorParameterValueOnAllMeshComponents(TargetActor, ParameterName, FVector(ParameterValue), bIncludeChildActors);
}

TArray<UActorComponent*> UXYSBlueprintFunctionLibrary::FindComponentsByClass(AActor* TargetActor, TSubclassOf<UActorComponent> ComponentClass, bool bIncludeChildActors)
{
