...
UXYSReplaySubsystem::UXYSReplaySubsystem()
{
}

bool UXYSReplaySubsystem::DoesPlatformSupportReplays()
{
...
FGameplayTag UXYSReplaySubsystem::GetPlatformSupportTraitTag()
{
	return TAG_Platform_Trait_ReplaySupport.GetTag();
}

void UXYSReplaySubsystem::PlayReplay(UXYSReplayListEntry* Replay)
{
	if (Replay != nullptr)
	{
...
void UXYSReplaySubsystem::RecordClientReplay(APlayerController* PlayerController)
{
	/*if (ensure(DoesPlatformSupportReplays() && PlayerController))
	{
		...
		if (UXYSLocalPlayer* XYSLocalPlayer = Cast<UXYSLocalPlayer>(PlayerController->GetLocalPlayer()))
		{
...
void UXYSReplaySubsystem::OnEnumerateStreamsCompleteForDelete(const FEnumerateStreamsResult& Result)
{
	...
	if (StreamsToDelete.Num() > DeletingReplaysNumberToKeep)
	{
		...
		UE_LOG(LogXYSGame, Log, TEXT("XYSReplaySubsystem asked to delete replay %s"), *ReplayName);
...
void UXYSReplaySubsystem::OnDeleteReplay(const FDeleteFinishedStreamResult& DeleteResult)
{
	if (!CurrentReplayStreamer.IsValid() || !IsValid(LocalPlayerDeletingReplays))
	{
		// Lost context, don't do anything
	...
	else
	{
		...
		UE_LOG(LogXYSGame, Warning, TEXT("Failed to delete replay with error %d!"), (int32)DeleteResult.Result);
...
void UXYSReplaySubsystem::SeekInActiveReplay(float TimeInSeconds)
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
float UXYSReplaySubsystem::GetReplayLengthInSeconds() const
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
float UXYSReplaySubsystem::GetReplayCurrentTime() const
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
UDemoNetDriver* UXYSReplaySubsystem::GetDemoDriver() const
{
