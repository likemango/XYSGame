...
bool UXYSAudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UXYSAudioMixEffectsSubsystem::PostInitialize()
{
	if (const UXYSAudioSettings* XYSAudioSettings = GetDefault<UXYSAudioSettings>())
	{
		if (UObject* ObjPath = XYSAudioSettings->DefaultControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Default Control Bus Mix reference missing from XYS Audio Settings."));
			}
		}

		if (UObject* ObjPath = XYSAudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from XYS Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = XYSAudioSettings->UserSettingsControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("User Control Bus Mix reference missing from XYS Audio Settings."));
			}
		}
		...
		for (const FXYSSubmixEffectChainMap& SoftSubmixEffectChain : XYSAudioSettings->HDRAudioSubmixEffectChain)
		{
			FXYSAudioSubmixEffectsChain NewEffectChain;
		...
		for (const FXYSSubmixEffectChainMap& SoftSubmixEffectChain : XYSAudioSettings->LDRAudioSubmixEffectChain)
		{
			FXYSAudioSubmixEffectsChain NewEffectChain;
...
void UXYSAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		/*if (const UXYSSettingsLocal* XYSSettingsLocal = GetDefault<UXYSSettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, XYSSettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, XYSSettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, XYSSettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, XYSSettingsLocal->GetDialogueVolume());
					const FSoundControlBusMixStage VoiceChatControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, XYSSettingsLocal->GetVoiceChatVolume());
...
void UXYSAudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<FXYSAudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<FXYSAudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const FXYSAudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const FXYSAudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void UXYSAudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void UXYSAudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool UXYSAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
