...
#include "Settings/CustomSettings/XYSSettingValueDiscrete_OverallQuality.h"
#include "Settings/CustomSettings/XYSSettingValueDiscrete_Resolution.h"
...
{
public:
	FGameSettingEditCondition_FramePacingMode(EXYSFramePacingMode InDesiredMode, EFramePacingEditCondition InMatchMode = EFramePacingEditCondition::EnableIf)
	...
	{
		const EXYSFramePacingMode ActualMode = UXYSPlatformSpecificRenderingSettings::Get()->FramePacingMode;
...
private:
	EXYSFramePacingMode DesiredMode;
...
public:
	...
	virtual void GatherEditState(const ULocalPlayer* InLocalPlayer, FGameSettingEditableState& InOutEditState) const override
	{
		if (!UXYSPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings)
		{
	...
	virtual void SettingChanged(const ULocalPlayer* LocalPlayer, UGameSetting* Setting, EGameSettingChangeReason Reason) const override
	{
		...
		const UXYSLocalPlayer* XYSLocalPlayer = CastChecked<UXYSLocalPlayer>(LocalPlayer);
		XYSLocalPlayer->GetLocalSettings()->ApplyScalabilitySettings();
	}
...
UGameSettingCollection* UXYSGameSettingRegistry::InitializeVideoSettings(UXYSLocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			UXYSSettingValueDiscrete_Resolution* Setting = NewObject<UXYSSettingValueDiscrete_Resolution>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UXYSSettingsShared>()->GetColorBlindMode());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UXYSSettingsShared>()->GetColorBlindStrength());
		...
		//----------------------------------------------------------------------------------
		{
			UXYSSettingAction_SafeZoneEditor* Setting = NewObject<UXYSSettingAction_SafeZoneEditor>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			for (const FXYSQualityDeviceProfileVariant& Variant : PlatformSettings->UserFacingDeviceProfileOptions)
			{
		...
		//----------------------------------------------------------------------------------
		{
			...
			UXYSSettingValueDiscrete_MobileFPSType* Setting = NewObject<UXYSSettingValueDiscrete_MobileFPSType>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EXYSFramePacingMode::MobileStyle));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetCustomAction([](ULocalPlayer* LocalPlayer)
			{
				...
				{
					UXYSSettingsLocal::Get()->ResetToMobileDeviceDefaults();
				}
				else
				{
					const UXYSLocalPlayer* XYSLocalPlayer = CastChecked<UXYSLocalPlayer>(LocalPlayer);
					...
					XYSLocalPlayer->GetLocalSettings()->RunAutoBenchmark(bImmediatelySaveState);
				}
			});
			...
			{
				const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
				const bool bCanUseDueToMobile = (PlatformSettings->FramePacingMode == EXYSFramePacingMode::MobileStyle);

				const UXYSLocalPlayer* XYSLocalPlayer = CastChecked<UXYSLocalPlayer>(LocalPlayer);
				const bool bCanBenchmark = XYSLocalPlayer->GetLocalSettings()->CanRunAutoBenchmark();
		...
		//----------------------------------------------------------------------------------
		{
			UXYSSettingValueDiscrete_OverallQuality* Setting = NewObject<UXYSSettingValueDiscrete_OverallQuality>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EXYSFramePacingMode::ConsoleStyle, EFramePacingEditCondition::DisableIf));
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EXYSFramePacingMode::DesktopStyle));
...
void AddFrameRateOptions(UGameSettingValueDiscreteDynamic_Number* Setting)
{
	...
	for (int32 Rate : GetDefault<UXYSPerformanceSettings>()->DesktopFrameRateLimits)
	{
...
void UXYSGameSettingRegistry::InitializeVideoSettings_FrameRates(UGameSettingCollection* Screen, UXYSLocalPlayer* InLocalPlayer)
{
	//----------------------------------------------------------------------------------
	{
		UGameSettingValueDiscreteDynamic_Number* Setting = NewObject<UGameSettingValueDiscreteDynamic_Number>();
		...
		Setting->SetDefaultValue(GetDefault<UXYSSettingsLocal>()->GetFrameRateLimit_OnBattery());
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UXYSSettingsLocal>()->GetFrameRateLimit_InMenu());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EXYSFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UXYSSettingsLocal>()->GetFrameRateLimit_WhenBackgrounded());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EXYSFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UXYSSettingsLocal>()->GetFrameRateLimit_Always());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(EXYSFramePacingMode::DesktopStyle));
