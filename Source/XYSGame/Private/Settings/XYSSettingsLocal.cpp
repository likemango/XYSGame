...
#include "Audio/XYSAudioMixEffectsSubsystem.h"
#include "Audio/XYSAudioSettings.h"
#include "Development/XYSPlatformEmulationSettings.h"
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("XYS.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("XYS.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("XYS.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("XYS.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("XYS.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("XYS.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("XYS.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("XYS.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("XYS.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("XYS.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FXYSScalabilitySnapshot::FXYSScalabilitySnapshot()
{
...
namespace XYSSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FXYSScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UXYSSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UXYSSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UXYSSettingsLocal::UXYSSettingsLocal()
{
	...
	bEnableScalabilitySettings = UXYSPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings;

	SetToDefaults();
}

void UXYSSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
	...
	bEnableLatencyTrackingStats = UXYSSettingsLocal::DoesPlatformSupportLatencyTrackingStats();
...
void UXYSSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EXYSFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	XYSSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UXYSSettingsLocal::ResetToCurrentSettings()
{
...
void UXYSSettingsLocal::BeginDestroy()
{
...
UXYSSettingsLocal* UXYSSettingsLocal::Get()
{
	return GEngine ? CastChecked<UXYSSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UXYSSettingsLocal::ConfirmVideoMode()
{
...
float UXYSSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EXYSFramePacingMode::DesktopStyle)
	{
...
int32 UXYSSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return XYSSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UXYSSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FXYSScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UXYSSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UXYSSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UXYSSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UXYSSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UXYSSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UXYSSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EXYSStatDisplayMode UXYSSettingsLocal::GetPerfStatDisplayState(EXYSDisplayablePerformanceStat Stat) const
{
	if (const EXYSStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EXYSStatDisplayMode::Hidden;
	}
}

void UXYSSettingsLocal::SetPerfStatDisplayState(EXYSDisplayablePerformanceStat Stat, EXYSStatDisplayMode DisplayMode)
{
	if (DisplayMode == EXYSStatDisplayMode::Hidden)
	{
...
bool UXYSSettingsLocal::DoesPlatformSupportLatencyMarkers()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyMarkers);
}

void UXYSSettingsLocal::SetEnableLatencyFlashIndicators(const bool bNewVal)
{
...
void UXYSSettingsLocal::SetEnableLatencyTrackingStats(const bool bNewVal)
{
...
void UXYSSettingsLocal::ApplyLatencyTrackingStatSetting()
{
...
bool UXYSSettingsLocal::DoesPlatformSupportLatencyTrackingStats()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyStats);
}

float UXYSSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UXYSSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UXYSSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UXYSSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UXYSSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UXYSSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UXYSSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UXYSSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UXYSSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UXYSSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UXYSSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UXYSSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UXYSSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UXYSSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UXYSSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UXYSSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return XYSSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UXYSSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return XYSSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UXYSSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UXYSSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EXYSFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return XYSSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UXYSSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EXYSFramePacingMode::MobileStyle)
	{
...
void UXYSSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UXYSSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EXYSFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = XYSSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UXYSSettingsLocal::ClampMobileQuality()
{
	const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EXYSFramePacingMode::MobileStyle)
	{
...
void UXYSSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = XYSSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UXYSSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = XYSSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = XYSSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UXYSSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UXYSSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UXYSSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UXYSSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UXYSSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = XYSSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UXYSSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UXYSSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UXYSAudioMixEffectsSubsystem* XYSAudioMixEffectsSubsystem = World->GetSubsystem<UXYSAudioMixEffectsSubsystem>())
			{
				XYSAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UXYSSettingsLocal::CanRunAutoBenchmark() const
{
	const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UXYSSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UXYSSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UXYSSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UXYSSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UXYSSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UXYSSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UXYSSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UXYSSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UXYSSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UXYSSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UXYSSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UXYSSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UXYSSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UXYSSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UXYSSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UXYSSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UXYSSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UXYSSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UXYSSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EXYSFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = XYSSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UXYSSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UXYSSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UXYSSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UXYSAudioSettings* XYSAudioSettings = GetDefault<UXYSAudioSettings>())
			{
				...
				if (UObject* ObjPath = XYSAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from XYS Audio Settings."));
					}
				}

				if (UObject* ObjPath = XYSAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from XYS Audio Settings."));
					}
				}

				if (UObject* ObjPath = XYSAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from XYS Audio Settings."));
					}
				}

				if (UObject* ObjPath = XYSAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from XYS Audio Settings."));
					}
				}

				if (UObject* ObjPath = XYSAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from XYS Audio Settings."));
					}
				}

				if (UObject* ObjPath = XYSAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from XYS Audio Settings."));
					}
				}
			}
		}
...
void UXYSSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UXYSSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UXYSPlatformSpecificRenderingSettings* PlatformSettings = UXYSPlatformSpecificRenderingSettings::Get();
	const TArray<FXYSQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FXYSQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UXYSPlatformEmulationSettings* Settings = GetDefault<UXYSPlatformEmulationSettings>();
	...
	{
	case EXYSFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EXYSFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EXYSFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UXYSSettingsLocal::UpdateConsoleFramePacing()
{
...
void UXYSSettingsLocal::UpdateDesktopFramePacing()
{
...
void UXYSSettingsLocal::UpdateMobileFramePacing()
{
...
void UXYSSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
