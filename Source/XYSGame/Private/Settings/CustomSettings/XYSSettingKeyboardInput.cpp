...
#include "Settings/CustomSettings/XYSSettingKeyboardInput.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(XYSSettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "XYSSettings"

namespace XYS::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("XYSErrors_UnknownMappingName", "Unknown Mapping");
}


UXYSSettingKeyboardInput::UXYSSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText UXYSSettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return XYS::ErrorMessages::UnknownMappingName;
}

FText UXYSSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* UXYSSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* UXYSSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* UXYSSettingKeyboardInput::GetUserSettings() const
{
	if(UXYSLocalPlayer* XYSLocalPlayer = Cast<UXYSLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = XYSLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void UXYSSettingKeyboardInput::OnInitialized()
{
...
void UXYSSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText UXYSSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
void UXYSSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void UXYSSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void UXYSSettingKeyboardInput::RestoreToInitial()
{
...
bool UXYSSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void UXYSSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool UXYSSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
