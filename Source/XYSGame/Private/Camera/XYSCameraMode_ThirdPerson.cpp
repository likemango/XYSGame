...
#include "Camera/XYSCameraMode_ThirdPerson.h"
#include "Camera/XYSCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/XYSPenetrationAvoidanceFeeler.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(XYSCameraMode_ThirdPerson)

namespace XYSCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UXYSCameraMode_ThirdPerson::UXYSCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FXYSPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FXYSPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FXYSPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FXYSPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FXYSPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FXYSPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FXYSPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UXYSCameraMode_ThirdPerson::OnActivation()
{
	if (AXYSCharacter* Character = Cast<AXYSCharacter>(GetTargetActor()))
	{
		Character->SetCharacterMeshViewMode(ECharacterViewType::ThirdPerson);
	}
}

void UXYSCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UXYSCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
	if (AXYSCharacter* TargetCharacter = Cast<AXYSCharacter>(GetTargetActor()))
	{
		if (TargetCharacter->IsCrouched())
		{
			const AXYSCharacter* TargetCharacterCDO = TargetCharacter->GetClass()->GetDefaultObject<AXYSCharacter>();
...
void UXYSCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UXYSCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IXYSCameraAssistInterface* TargetControllerAssist = Cast<IXYSCameraAssistInterface>(TargetController);

	IXYSCameraAssistInterface* TargetActorAssist = Cast<IXYSCameraAssistInterface>(TargetActor);
	...
	IXYSCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IXYSCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IXYSCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IXYSCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UXYSCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IXYSCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FXYSPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(XYSCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UXYSCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UXYSCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
