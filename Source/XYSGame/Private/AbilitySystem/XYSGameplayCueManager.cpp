...
#include UE_INLINE_GENERATED_CPP_BY_NAME(XYSGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EXYSEditorLoadMode
{
...
namespace XYSGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("XYS.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via XYSGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UXYSGameplayCueManager::DumpGameplayCues));

	static EXYSEditorLoadMode LoadMode = EXYSEditorLoadMode::LoadUpfront;
}
...
UXYSGameplayCueManager::UXYSGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UXYSGameplayCueManager* UXYSGameplayCueManager::Get()
{
	return Cast<UXYSGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UXYSGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UXYSGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogXYSGame, Warning, TEXT("UXYSGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UXYSGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (XYSGameplayCueManagerCvars::LoadMode)
	{
	case EXYSEditorLoadMode::LoadUpfront:
		return true;
	case EXYSEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EXYSEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UXYSGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UXYSGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UXYSGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UXYSGameplayCueManager* GCM = Cast<UXYSGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogXYSGame, Error, TEXT("DumpGameplayCues failed. No UXYSGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogXYSGame, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogXYSGame, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogXYSGame, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogXYSGame, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogXYSGame, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogXYSGame, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogXYSGame, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogXYSGame, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogXYSGame, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogXYSGame, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogXYSGame, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogXYSGame, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UXYSGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UXYSGameplayCueManager* StrongThis = Get())
					{
...
void UXYSGameplayCueManager::HandlePostGarbageCollect()
{
...
void UXYSGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogXYSGame, Warning, TEXT("UXYSGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UXYSGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (XYSGameplayCueManagerCvars::LoadMode)
	{
	case EXYSEditorLoadMode::LoadUpfront:
...
void UXYSGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UXYSGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UXYSGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UXYSGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UXYSGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UXYSGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
