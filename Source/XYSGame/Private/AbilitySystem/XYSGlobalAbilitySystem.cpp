...
#include UE_INLINE_GENERATED_CPP_BY_NAME(XYSGlobalAbilitySystem)

void FGlobalAppliedAbilityList::AddToASC(TSubclassOf<UGameplayAbility> Ability, UXYSAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedAbilityList::RemoveFromASC(UXYSAbilitySystemComponent* ASC)
{
	if (FGameplayAbilitySpecHandle* SpecHandle = Handles.Find(ASC))
	{
...
void FGlobalAppliedEffectList::AddToASC(TSubclassOf<UGameplayEffect> Effect, UXYSAbilitySystemComponent* ASC)
{
...
void FGlobalAppliedEffectList::RemoveFromASC(UXYSAbilitySystemComponent* ASC)
{
	if (FActiveGameplayEffectHandle* EffectHandle = Handles.Find(ASC))
	{
...
UXYSGlobalAbilitySystem::UXYSGlobalAbilitySystem()
{
}

void UXYSGlobalAbilitySystem::ApplyAbilityToAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && (!AppliedAbilities.Contains(Ability)))
	{
		FGlobalAppliedAbilityList& Entry = AppliedAbilities.Add(Ability);		
		for (UXYSAbilitySystemComponent* ASC : RegisteredASCs)
		{
...
void UXYSGlobalAbilitySystem::ApplyEffectToAll(TSubclassOf<UGameplayEffect> Effect)
{
...
void UXYSGlobalAbilitySystem::RemoveAbilityFromAll(TSubclassOf<UGameplayAbility> Ability)
{
	if ((Ability.Get() != nullptr) && AppliedAbilities.Contains(Ability))
	{
...
void UXYSGlobalAbilitySystem::RemoveEffectFromAll(TSubclassOf<UGameplayEffect> Effect)
{
	if ((Effect.Get() != nullptr) && AppliedEffects.Contains(Effect))
	{
...
void UXYSGlobalAbilitySystem::RegisterASC(UXYSAbilitySystemComponent* ASC)
{
	check(ASC);
...
void UXYSGlobalAbilitySystem::UnregisterASC(UXYSAbilitySystemComponent* ASC)
{
	check(ASC);
	for (auto& Entry : AppliedAbilities)
	{
