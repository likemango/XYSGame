...
#include "AbilitySystem/Executions/XYSDamageExecution.h"
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UXYSCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
UXYSDamageExecution::UXYSDamageExecution()
{
	RelevantAttributesToCapture.Add(DamageStatics().BaseDamageDef);
}

void UXYSDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FXYSGameplayEffectContext* TypedContext = FXYSGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	// TODO: Apply rules for team damage/self damage/etc...
	float DamageInteractionAllowedMultiplier = 1.0f;
	// if (HitActor)
	// {
	// 	UXYSTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UXYSTeamSubsystem>();
	...
	{
		UE_LOG(LogXYSAbilitySystem, Error, TEXT("Damage Calculation cannot deduce a source location for damage coming from %s; Falling back to WORLD_MAX dist!"), *GetPathNameSafe(Spec.Def));
	}

	// Apply ability source modifiers
	float PhysicalMaterialAttenuation = 1.0f;
	float DistanceAttenuation = 1.0f;
	if (const IXYSAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	{
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UXYSHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
