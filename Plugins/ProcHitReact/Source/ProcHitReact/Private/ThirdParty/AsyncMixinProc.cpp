...
DEFINE_LOG_CATEGORY_STATIC(LogAsyncMixinProc, Log, All);

TMap<FAsyncMixinProc*, TSharedRef<FAsyncMixinProc::FLoadingState>> FAsyncMixinProc::Loading;

FAsyncMixinProc::FAsyncMixinProc()
{
}
...
const FAsyncMixinProc::FLoadingState& FAsyncMixinProc::GetLoadingStateConst() const
{
	check(IsInGameThread());
	return Loading.FindChecked(this).Get();
}

FAsyncMixinProc::FLoadingState& FAsyncMixinProc::GetLoadingState()
{
	check(IsInGameThread());
...
bool FAsyncMixinProc::HasLoadingState() const
{
	check(IsInGameThread());
...
void FAsyncMixinProc::CancelAsyncLoading()
{
...
bool FAsyncMixinProc::IsAsyncLoadingInProgress() const
{
	// Don't create the loading state if we don't have anything pending.
	if (HasLoadingState())
	{
...
bool FAsyncMixinProc::IsLoadingInProgressOrPending() const
{
	if (HasLoadingState())
	{
...
void FAsyncMixinProc::AsyncLoad(FSoftObjectPath SoftObjectPath, const FSimpleDelegate& DelegateToCall)
{
	GetLoadingState().AsyncLoad(SoftObjectPath, DelegateToCall);
}

void FAsyncMixinProc::AsyncLoad(const TArray<FSoftObjectPath>& SoftObjectPaths, const FSimpleDelegate& DelegateToCall)
{
	GetLoadingState().AsyncLoad(SoftObjectPaths, DelegateToCall);
}

void FAsyncMixinProc::AsyncPreloadPrimaryAssetsAndBundles(const TArray<FPrimaryAssetId>& AssetIds, const TArray<FName>& LoadBundles, const FSimpleDelegate& DelegateToCall)
{
	GetLoadingState().AsyncPreloadPrimaryAssetsAndBundles(AssetIds, LoadBundles, DelegateToCall);
}

void FAsyncMixinProc::AsyncCondition(TSharedRef<FAsyncCondition> Condition, const FSimpleDelegate& Callback)
{
	GetLoadingState().AsyncCondition(Condition, Callback);
}

void FAsyncMixinProc::AsyncEvent(const FSimpleDelegate& Callback)
{
	GetLoadingState().AsyncEvent(Callback);
}

void FAsyncMixinProc::StartAsyncLoading()
{
...
FAsyncMixinProc::FLoadingState::FLoadingState(FAsyncMixinProc& InOwner)
	: OwnerRef(InOwner)
{
}

FAsyncMixinProc::FLoadingState::~FLoadingState()
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_FAsyncMixinProc_FLoadingState_DestroyThisMemoryDelegate);
	UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] Destroy LoadingState (Done)"), this);
...
void FAsyncMixinProc::FLoadingState::CancelOnly(bool bDestroying)
{
	if (!bDestroying)
	{
		UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] Cancel"), this);
	}
...
void FAsyncMixinProc::FLoadingState::CancelAndDestroy()
{
	CancelOnly(/*bDestroying*/false);
	RequestDestroyThisMemory();
}

void FAsyncMixinProc::FLoadingState::CancelDestroyThisMemory(bool bDestroying)
{
	...
	if (IsPendingDestroy())
	{
		if (!bDestroying)
		{
			UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] Destroy LoadingState (Canceled)"), this);
		}
...
void FAsyncMixinProc::FLoadingState::RequestDestroyThisMemory()
{
	// If we're already pending to destroy this memory, just ignore.
	if (!IsPendingDestroy())
	{
		UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] Destroy LoadingState (Requested)"), this);
		...
		DestroyMemoryDelegate = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda([this](float DeltaTime) {
			...
			FAsyncMixinProc::Loading.Remove(&OwnerRef);
			return false;
		}));
	}
}

void FAsyncMixinProc::FLoadingState::CancelStartTimer()
{
...
void FAsyncMixinProc::FLoadingState::Start()
{
	UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] Start (Current Progress %d/%d)"), this, CurrentAsyncStep + 1, AsyncSteps.Num());
...
void FAsyncMixinProc::FLoadingState::AsyncLoad(FSoftObjectPath SoftObjectPath, const FSimpleDelegate& DelegateToCall)
{
	UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] AsyncLoad '%s'"), this, *SoftObjectPath.ToString());

	AsyncSteps.Add(
		MakeUnique<FAsyncStep>(
			DelegateToCall,
			UAssetManager::GetStreamableManager().RequestAsyncLoad(SoftObjectPath, FStreamableDelegate(), FStreamableManager::AsyncLoadHighPriority, false, false, TEXT("AsyncMixinProc"))
			)
	);
...
void FAsyncMixinProc::FLoadingState::AsyncLoad(const TArray<FSoftObjectPath>& SoftObjectPaths, const FSimpleDelegate& DelegateToCall)
{
	{
		...
		UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] AsyncLoad [%s]"), this, *Paths);
	}

	AsyncSteps.Add(
		MakeUnique<FAsyncStep>(
			DelegateToCall,
			UAssetManager::GetStreamableManager().RequestAsyncLoad(SoftObjectPaths, FStreamableDelegate(), FStreamableManager::AsyncLoadHighPriority, false, false, TEXT("AsyncMixinProc"))
			)
	);
...
void FAsyncMixinProc::FLoadingState::AsyncPreloadPrimaryAssetsAndBundles(const TArray<FPrimaryAssetId>& AssetIds, const TArray<FName>& LoadBundles, const FSimpleDelegate& DelegateToCall)
{
	{		
		...
		UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X]  AsyncPreload Assets [%s], Bundles[%s]"), this, *Assets, *Bundles);
	}
...
void FAsyncMixinProc::FLoadingState::AsyncCondition(TSharedRef<FAsyncCondition> Condition, const FSimpleDelegate& DelegateToCall)
{
	UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] AsyncCondition '0x%X'"), this, &Condition.Get());
...
void FAsyncMixinProc::FLoadingState::AsyncEvent(const FSimpleDelegate& DelegateToCall)
{
	UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] AsyncEvent"), this);
...
void FAsyncMixinProc::FLoadingState::TryScheduleStart()
{
	...
	if (!StartTimerDelegate.IsValid())
	{
		StartTimerDelegate = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda([this](float DeltaTime) {
			QUICK_SCOPE_CYCLE_COUNTER(STAT_FAsyncMixinProc_FLoadingState_TryScheduleStartDelegate);
			Start();
			return false;
		}));
	}
...
bool FAsyncMixinProc::FLoadingState::IsLoadingInProgress() const
{
	if (AsyncSteps.Num() > 0)
	{
...
bool FAsyncMixinProc::FLoadingState::IsLoadingInProgressOrPending() const
{
	return StartTimerDelegate.IsValid() || IsLoadingInProgress();
}

bool FAsyncMixinProc::FLoadingState::IsPendingDestroy() const
{
	return DestroyMemoryDelegate.IsValid();
}

void FAsyncMixinProc::FLoadingState::TryCompleteAsyncLoading()
{
	...
	UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] TryCompleteAsyncLoading - (Current Progress %d/%d)"), this, CurrentAsyncStep + 1, AsyncSteps.Num());
	...
	while (CurrentAsyncStep < AsyncSteps.Num())
	{
		...
		if (Step->IsLoadingInProgress())
		{
			...
			{
				UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] Step %d - Still Loading (Listening)"), this, CurrentAsyncStep + 1);
			...
			{
				UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] Step %d - Still Loading (Waiting)"), this, CurrentAsyncStep + 1);
			}
		...
		{
			UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] Step %d - Completed (Calling User)"), this, CurrentAsyncStep + 1);
...
void FAsyncMixinProc::FLoadingState::CompleteAsyncLoading()
{
	UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] CompleteAsyncLoading"), this);
...
FAsyncMixinProc::FLoadingState::FAsyncStep::FAsyncStep(const FSimpleDelegate& InUserCallback)
	: UserCallback(InUserCallback)
{
}

FAsyncMixinProc::FLoadingState::FAsyncStep::FAsyncStep(const FSimpleDelegate& InUserCallback, const TSharedPtr<FStreamableHandle>& InStreamingHandle)
	: UserCallback(InUserCallback)
	, StreamingHandle(InStreamingHandle)
{
}

FAsyncMixinProc::FLoadingState::FAsyncStep::FAsyncStep(const FSimpleDelegate& InUserCallback, const TSharedPtr<FAsyncCondition>& InCondition)
	: UserCallback(InUserCallback)
	, Condition(InCondition)
{
}

FAsyncMixinProc::FLoadingState::FAsyncStep::~FAsyncStep()
{

}

void FAsyncMixinProc::FLoadingState::FAsyncStep::ExecuteUserCallback()
{
	UserCallback.ExecuteIfBound();
	UserCallback.Unbind();
}

bool FAsyncMixinProc::FLoadingState::FAsyncStep::IsComplete() const
{
...
void FAsyncMixinProc::FLoadingState::FAsyncStep::Cancel()
{
	if (StreamingHandle.IsValid())
	{
...
bool FAsyncMixinProc::FLoadingState::FAsyncStep::BindCompleteDelegate(const FSimpleDelegate& NewDelegate)
{
	if (IsComplete())
	{
		// Too Late!
...
bool FAsyncMixinProc::FLoadingState::FAsyncStep::IsCompleteDelegateBound() const
{
...
bool FAsyncCondition::TryToContinue(float)
{
	...
	UE_LOG(LogAsyncMixinProc, Verbose, TEXT("[0x%X] AsyncCondition::TryToContinue"), this);
