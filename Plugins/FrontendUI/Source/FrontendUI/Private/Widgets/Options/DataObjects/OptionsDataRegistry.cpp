...
#define GET_DESCRIPTION(InKey) LOCTABLE("/FrontendUI/Wigets/StringTables/ST_OptionsScreenDescription.ST_OptionsScreenDescription", InKey)
...
void UOptionsDataRegistry::InitGameplayCollectionTab()
{
	...
	//Test Item 2
	{
		...
		TestItem->SetDescriptionRichText(FText::FromString(TEXT("The is a test")));
...
void UOptionsDataRegistry::InitVideoCollectionTab()
{
	...
	//Display CategoryAdd commentMore actions
	{
		...
		//Window Mode
		{
			...
			// const FText WindowModeDesc = LOCTABLE("/FrontendUI/Wigets/StringTables/ST_OptionsScreenDescription.ST_OptionsScreenDescription", "WindowModeDescKey");
		...
		//Screen ResolutionAdd commentMore actions
		{
			...
			for (const TPair<FString, UEnhancedPlayerMappableKeyProfile*>& ProfilePair : EIUserSettings->GetAllAvailableKeyProfiles())
			{
				...
				for (const TPair<FName, FKeyMappingRow>& MappingRowPair : MappableKeyProfile->GetPlayerMappingRows())
				{
					for (const FPlayerKeyMapping& KeyMapping : MappingRowPair.Value.Mappings)
					{
						...
						else if (MappableKeyProfile->DoesMappingPassQueryOptions(KeyMapping, GamepadOnly))
						{
							UListDataObject_KeyRemap* KeyRemapDataObject = NewObject<UListDataObject_KeyRemap>();
							KeyRemapDataObject->SetDataID(KeyMapping.GetMappingName());
							KeyRemapDataObject->SetDataDisplayName(KeyMapping.GetDisplayName());
